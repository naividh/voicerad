...
function ConversationTurn({ turn, index }) {
  return (
    <div className="turn">
      ...
      {turn.safety && <SafetyBanner safety={turn.safety} compact />}
    </div>
  );
}
...
function InterpretationView({
  ...
  return (
    <div className="results">
      ...
      {/* Safety Banner - always shown above interpretation */}
      <SafetyBanner safety={safety} />

      <pre className="report">{interpretation}</pre>

      {/* Clarifying questions from safety assessment */}
      {sessionActive && (
        <ClarifyingQuestions questions={clarifyingQuestions} onSelect={onSelectQuestion} />
      )}
...
export default function App() {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      setTurns((prev) => [
        ...prev,
        {
          input: continueText.trim(),
          response: data.refined_interpretation,
  ...
  return (
    <div className="vr">
      ...
      <footer className="ft">
        <p>⚠️ VoiceRad is an AI-assisted tool. All interpretations require clinician review and sign-off.</p>
        <p>Powered by Google MedGemma | Clinical Safety Rails v1.3</p>
      </footer>
    </div>
  );
}
